...
it('can render the view page', function (): void {
    $record = Deal::factory()->recycle([$this->user, $this->team])->create();

    livewire(ViewDeal::class, ['record' => $record->getKey()])
        ->assertOk();
});
...
it('exposes the expected table columns', function (): void {
    $table = livewire(ListDeals::class);
...
it('can sort `:dataset` column', function (string $column): void {
    $records = Deal::factory(3)->recycle([$this->user, $this->team])->create();
...
it('cannot display trashed records by default', function (): void {
    $records = Deal::factory()->count(4)->recycle([$this->user, $this->team])->create();
    $trashedRecords = Deal::factory()->trashed()->count(6)->recycle([$this->user, $this->team])->create();
...
it('can paginate records', function (): void {
    $records = Deal::factory(20)->recycle([$this->user, $this->team])->create();
...
it('can bulk delete records', function (): void {
    $records = Deal::factory(5)->recycle([$this->user, $this->team])->create();
...
it('can edit an deal', function (): void {
    $record = Deal::factory()->recycle([$this->user, $this->team])->create();
    ...
    expect($record->refresh()->name)->toBe('Updated Deal');
});
...
it('sets creator_id and team_id via observer when creating an deal', function (): void {
    livewire(ListDeals::class)
        ->callAction('create', data: [
    ...
    $deal = Deal::query()->where('name', 'Observer Test Deal')->first();

    expect($deal->creator_id)->toBe($this->user->id)
        ->and($deal->team_id)->toBe($this->team->id);
});

it('authorizes team member to view and update own team deal', function (): void {
    $record = Deal::factory()->recycle([$this->user, $this->team])->create();
...
it('denies non-team-member from viewing another team deal', function (): void {
    $otherUser = User::factory()->withTeam()->create();
    $otherTeam = $otherUser->currentTeam;

    $this->actingAs($otherUser);
    $record = Deal::factory()->for($otherTeam)->create();
    $this->actingAs($this->user);
