...
use Relaticle\Chat\Tools\Deal\CreateDealTool;
use Relaticle\Chat\Tools\Deal\UpdateDealTool;

mutates(CreateDealTool::class);
mutates(UpdateDealTool::class);
mutates(CreateDeal::class);
...
it('CreateDealTool wraps entity fields inside a records[] schema', function (): void {
    $tool = resolve(CreateDealTool::class);
...
it('approving an deal creates it linked to a company and contact', function (): void {
    $company = Company::factory()->for($this->team)->create(['name' => 'Acme']);
    $contact = People::factory()->for($this->team)->create(['name' => 'Angel']);

    $deal = resolve(CreateDeal::class)->execute(
        $this->user,
        [
            'name' => 'Acme deal',
    ...
    expect((string) $deal->company_id)->toBe((string) $company->id);
    expect((string) $deal->contact_id)->toBe((string) $contact->id);
});
...
it('rejects cross-tenant company_id at the action layer', function (): void {
    ...
    expect(fn () => resolve(CreateDeal::class)->execute(
        $this->user,
...
it('UpdateDealTool renders linked names in the update proposal display data', function (): void {
