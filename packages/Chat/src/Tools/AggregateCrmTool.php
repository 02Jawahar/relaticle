...
final class AggregateCrmTool implements Tool
{
    ...
    {
        return 'Aggregate deals by stage or company: count and total pipeline value per group, with optional date range. Use for "pipeline by stage", "deals by company", "total value", "how many deals" questions.';
    }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'date_from' => $schema->string()->description('Optional start date (YYYY-MM-DD). Only include deals created on or after this date.'),
            'date_to' => $schema->string()->description('Optional end date (YYYY-MM-DD). Only include deals created on or before this date.'),
        ];
    }
    ...
    public function handle(Request $request): string
    {
        ...
        try {
            $result = resolve(AggregateDeals::class)->execute(
                user: $user,
                groupBy: $groupBy,
                dateFrom: $dateFrom,
                dateTo: $dateTo,
